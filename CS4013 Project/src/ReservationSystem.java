...
public class ReservationSystem {
	...
	/***
	 * Displays analytics to the user if administrator
	 */
	public void getAnalysis() {
		System.out.println("ANALYSIS");
		System.out.println("Please enter a date from which to start analysis.");
		LocalDate start = inputDate();
		System.out.println("Please enter a date at which to end analysis.");
		LocalDate end = inputDate();
		
		for (int i = 0; i < hotels.size(); i++) {
			System.out.println(hotels.get(i).toString() + ": €" + Analytics.income(start, end, hotels.get(i), reservations));
			for (int j = 0; j < hotels.get(i).rooms.size(); j++) {
				System.out.println(hotels.get(i).rooms.get(j).toString() + ": " + Analytics.occupancy(start, end, hotels.get(i), reservations) + " people");
			}
			System.out.println();
		}
	}
